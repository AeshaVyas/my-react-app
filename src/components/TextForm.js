...
export default function TextForm(props) {
    ...
    const handleClearClick = () => {
        ...
        setText(newText);
        props.showAlert("Reverse the sentence!","success");
...
return (
    ...
    <div className='container' style={{color:props.mode === 'dark'? 'white' :'black'}}>
        ...
        <button className="btn btn-primary mx-2" onClick={handleClearClick}>Reverse the Text</button>
        <button className="btn btn-primary mx-2" onClick={handleCopy}>Copy Text</button>
        <button className="btn btn-primary mx-2" onClick={handleExtraSpaces}>Remove Extra Spacest</button>

    </div>
    <div className="container my-2" style={{color:props.mode === 'dark'? 'white' :'black'}}>
        <h2>Your text summary</h2>
        <p>{text.trim() === "" ? 0 : text.trim().split(/\s+/).length} words and {text.length} characters</p>
        <p>{text.trim() === "" ? 0 : 0.08 * text.trim().split(/\s+/).length} minutes </p>
        <h2>Preview</h2>
